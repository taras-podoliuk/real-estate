              ...
              <TableRow key={payment.id} className="h-16">
                ...
                <TableCell>
                  <button className="border border-gray-300 text-gray-700 py-2 px-4 rounded-md flex items-center justify-center font-semibold hover:bg-primary-700 hover:text-primary-50">
                    <ArrowDownToLineIcon className="size-4 mr-1" />
                    Download
                  </button>
                </TableCell>
              </TableRow>
...
function Residence() {
  ...
  if (isPropertyLoading || isLeasesLoading || isPaymentsLoading)
    return <Loading />;
  if (!property || isPropertyError) return <div>Error loading property</div>;

  const currentLease = leases?.find((lease) => lease.id === property.id);
